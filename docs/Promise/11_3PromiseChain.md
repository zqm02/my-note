...
让我们将本章所讲的这个特性与在[上一章](https://zh.javascript.info/promise-basics#loadscript)中定义的 promise 化的`locaScript`结合使用，按顺序依次加载脚本:
...
在这儿，每个`loadScript`调用返回一个 promise，并且在它 resolve 时下一个`.then`开始运行。然后，它启动下一个脚本的加载。所以，脚本是一个接一个地加载地。
...
loadScript("/article/promise-chaining/one.js").then((script1) => {
  loadScript("/article/promise-chaining/two.js").then((script2) => {
    loadScript("/article/promise-chaining/three.js").then((script3) => {
      //此函数可以访问变量 script1，script2，script3
      one();
      two();
      three();
    });
...
有时候直接写`.then`也是可以的，因为嵌套的函数可以访问外部作用域。在上面的例子中，嵌套在最深层的那个回调(callback)可以访问所有变量`script1`,`script2`,`script3`。但这是一个例外，而不是一条规则。

> **Thenables**
> 确切的说，处理程序返回的不完全是一个 promise，而是返回的被称为"thenables"对象--一个具有方法`.then`的任意对象。它会被当作一个 promise 来对待。
>
> 这个想法是，第三方可可以实现自己的"promise 兼容(promise-compatible)"对象。它们可以具有扩展的方法集，但也与原生的 promise 兼容，因为它们实现了`.then`方法。
>
> 这是一个 thenable 对象的示例:
>
> ```js
...
> ```
>
> JavaScript 检查在`(*)`行中由`.then`处理程序返回的对象:如果它具有名为`then`的可调用方法，那么它将调用该方法提供原生的函数`resolve`和`reject`作为参数(类似于 executor)，并等待直到其中一个函数被调用。在上面的示例中，`resolve(2)`在 1 秒后被调用`(**)`。然后，result 会被进一步沿着链向下传递。
>
...
我们将一起使用[fetch](https://zh.javascript.info/fetch)方法从远程服务器加载用户信息。它有很多可选的参数，我们在[单独的一章](https://zh.javascript.info/fetch)中对其进行了详细介绍，但基本语法很简单:
...
fetch("/article/promise-chaining/user.json")
  ...
  .then(function (resonpse) {
    //当 user.json加载完成时，response.text() 会返回一个新的 promise
    //当 promise 以加载的 user.json 为result 进行 resolve
...
作为一个好的做法，异步行为应该始终返回一个 promise。这样就可以使得之后我们计划后续的行为成为可能。即时我们现在不打算对链进行扩展，但我们之后可能会需要。
...
![图片](../assert/imgs/PromiseChain3.png)
