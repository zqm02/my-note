...
- 下载外部模块脚本`<script type="module" src="...">`不会阻塞 HTML 的处理，他们会与其他资源并行加载。
...
```js
  ...
  <script>
  alert(typeof button); //button 为undefined,脚本看不到下面的元素
  //常规脚本会立即运行，常规脚本的运行是在处理的页面的其余部分之前进行的
  </script>
...
这是因为模块本是延迟的，所以要等到 HTML 文档被处理完成才会执行它。而常规代码则会立即运行，所以我们会先看到常规脚本的输出。

当使用模块脚本时，我们应该知道 HTML 页面在加载时就会显示出来，在 HTML 页面加载完成后才会执行 JavaScript 模块，因此用户可能会在 JavaScript 应用程序准备好之前看到该页面。某些功能那时可能还无法正使用。我们应该放置"加载指示器(loading indicarot)",否则，请确保不会使用户感到困惑。
...
它执行导入(fetch`./analytics.js`),并在导入完成时运行，即使 HTML 文档还未完成，或者其他脚本在等待处理中。

这对于不依赖任何其他东西的功能来说是非常棒的，例如计数器，广告，文档级事件监听器。

```
  <!-- 所有依赖都获取完成 (analytics.js) 然后脚本开始运行--->
  <!-- 不会等待HTML文档或者其他<script>标签-->
...
某些环境，想 Node.js 或者打包工具(bundle tool)允许没有任何路径的裸模块，因为它们有自己的查找模块的方法和钩子(hook)来对它们进行微调。但是浏览器尚不支持裸模块。
...
```js
  ...
  <script nomodule>
  alert("Modern browsers know both = module and nomodule,so skip this")
  alert("Old browsers ignore script with unknown type=module,execute this.")
```
...
使用打包工具的一个好处是--它们可以更好地控制模块的解析方式，允许我们使用裸模块和更多的功能，例如
CSS/HTML 模块等。

构建工具做以下这些事:

1. 从一个 打算放在 HTML 中的`<script type="module">`"主"模块开始。
...
```js
  <!--假设我们从诸如Webpack 这类的打包工具获得了 "bundler.js" 脚本-->
  <script src="bundler.js"></script>
```

关于构建工具说了这么多，但其实原生模块也是可以用的。所以，我们在这儿将不是使用 Webpack:你可以稍后再配置它。
