...
在数学里，可能会写为 x<sup>n</sup> = x \* x<sup>n-1</sup>。这叫做**一个递归步骤**：我们将任务转化为更简单的行为（`x`的乘法）和更简单的同类任务的调用（带有更小的`n`的`pow`运算）。接下来的步骤将其进一步简化，直到`n`达到`1`。
...
代码很短也容易理解（希望是这样？）。这就是递归的能力。它使用与任何层次的子部门嵌套。
