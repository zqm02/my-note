...
在前面的示例中，球可以杯被放置(drop)到"任何地方"。在实际中，我们通常是将一个元素放到另一个元素上。例如，将一个"文件"放置到一个"文件夹"或者其他地方。
...
这个解决方案很有意思，只是有点麻烦，所以我们在这对此进行介绍。】
...
这就是一开始的那个想法，将处理程序放在潜在的
"droppable"的元素，在实际操作总不起作用的原因。它们不会运行。
...
let elemBelow = document.elementFormPoint(event.clientX, event.clientY);
...
function onMouseMove(event) {
  ...
  //mousemove 事件可能会在窗口被触发 (当球被拖出屏幕时)
