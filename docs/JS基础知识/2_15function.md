...
如果我们需要更改消息或其显示方法，只需在一个地方修改代码：输出它的函数。
...
showMessage();
alert(userName); // John , 未被修改，函数没有访问外部变量。
```

> **全局变量**
> 在任何函数之外声明的变量，例如上述代码中的外部变量
> `userName`，都被称为**全局**变量。
...
当一个值被称作为函数参数(parameter)传递时，它也被称为**参数**(argument)。
换一种方式，我们把这些术语搞清楚:

- 参数(parameter)是函数声明中括号内列出的变量(它是函数声明时的术语)
- 参数(argument)是调用函数时传递给函数的值(它是函数调用时的术语)。

我们声明函数时列出它们的参数(parameter)，然后调用它们传递参数(argument)。
在上面的例子中，我们可以说："函数`showMessage`被声明，并且带有两个参数(parameter)，随后它被调用，两个参数(argument)分别为`from`和`"Hello"`"。

## 默认值

如果一个函数被调用，但有参数(argument)未被提供，那么相应的值就会变成`undefined`。
例如，之前提到的函数`showMessage(from,text)`可以只使用一个参数(agrument)调用:
...
我们可以使用`=`为函数声明中的参数值指定所谓的"默认"(如果对应参数的值未被传递则使用)值:
...
> 如果没有传递参数`text`，那么`anotherFunction()`就会被调用。

> **在 JavaScript 老代码中的默认参数**
> 几年前，JavaScript 不支持默认参数的语法。所以人们使用其他方法来设置默认参数。
> 如今，我们会在旧代码中看到它们。
> 例如，显式地检查`undefined`:
>
> ```js
...
我们可以通过将参数与`undefined`进行比较，来检查参数是否在函数执行期间被传递进来:
...
> ```js
> return;
> some + long + experssion + or + whatever * f(a) + f(b);
> ```
>
> 因此，实际上它的返回值变成了空值。
...
> 两个独立的行为通常需要两个函数，即使它们通常被一起使用(在这种情况下，我们可以创建第三个函数来调用这两个函数)。
> 有几个违反这一规则的例子:
>
...
>   这些例子假设没有函数名前缀具有通用含义。你和你的团队可以自定义这些函数前缀的含义，但是通常都没有太大的不同。无论怎样，你都应该对含税前缀的含义，带特定前缀的函数可以做什么及不可以做什么有深刻的了解。所有相同前缀的函数都应该遵守相同的规则。并且，团队成员应该形成共识。

> **非常短的函数命名**
> 常用的函数有时会有**非常短**的名字
> 例如，[jQuery](http://jquery.com/) 框架用`$`定义一个函数。[LoDash](http://lodash.com/)库的核心函数用`_`命名。
> 这些都是例外，一般而言，函数名应该简明扼要且具有描述性。
