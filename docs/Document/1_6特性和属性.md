...
但这里也有特例，例如`input.value`只能从特性同步到属性，反过来则不行:
...
有一种非常少见的情况。即使一个 DOM 属性是字符串类型的，但它可能和 HTML 特性也是不同的。例如,`href`DOM 属性一直是一个**完整**URL，即使该特性包含一个相对路径或者包含一个`#hash`。
...
<script>
  ...
  //属性
  alert(a.href); //http://site.com/page#hello 形式的完整URL
</script>
```
...
有时，非标注的特性常常用于将自定义的数据从 HTML 传递到 JavaScript，或者用于为 JavaScript"标记"HTML 元素。
...
但是自定义的特性也存在问题。如果我们出于我们的目的使用了非标注的特性，之后它被引入到了标准中并有了其自己的特性出现在了标准中，以满足开发者的需求。在这种情况下，自定义的属性可能会产生意料不到的影响。

为了避免冲突，存在[data-\*](https://html.spec.whatwg.org/#embedding-custom-non-visible-data-with-the-data-*-attributes)特性。

**所有以"data-\*"开头的特性均被保留程序员使用。它们可在`dataset`属性中使用。**
...
像`data-order-state`这样的多词性可以以驼峰式进行调用:`dataset.orderState`。
...
请注意，我么不仅可以读取数据，还可以修改数据属性(data-attribute)。然后 CSS 会更新相应的试图:在上面这个例子中的最后一行`(*)`将颜色更改为了蓝色。
...
|名字|名字(name)是大小写敏感的|名字(name)大小写不敏感|
...
在大多数下情况下，最好使用 DOM 属性。仅当 DOM 属性无法满足开发需求，并且我们真的需要特性时，才使用特性，例如:

- 我们需要一个非标注的特性。但是如果它以`data-`开头，那么我们应该使用`dataset`。
