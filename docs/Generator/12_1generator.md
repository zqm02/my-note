...
```js
  ...
  //迭代整个 range对象，返回从`range.from`到`range.to`范围的所有数字
  alert([...range]);  //1,2,3,4,5
```

我们可以通过提供一个 generator 函数作为
`Symbol.iterator`，来使用 generator 进行迭代:
...
现在返回一个 generator，而 generator 方法正式
`for..of`所期望的:
...
带有 generator 的变体比原来的`range`迭代代码简洁得多，
并且保持了相同的功能。
...
`yield*`指令将执行**委托**给另一个 generator。这个术语意味着`yield* gen`在 generator`gen`上进行迭代，并将其产出(yield)的值透明地(transparently)转发到外部。就好像这些值是由外部的 generator yield 的一样。
...
目前来看，generator 和可迭代对象类似，都具有用来生成值的特殊语法。但实际上，generator 更加强大且灵活。

这是因为`yield`是一条双向路(two-way street):它不仅可以向外返回结果，而且还可以将外部的值传递到 generator 内。

调用`generator.next(arg)`,我们呢就能将参数`arg`传递到 generator 内容。这个`arg`参数会变成`yield`的结果。
...
![picture](../assert/imgs/generator5.png)

1. 第一次调用`generator.next()`应该不是带参数的(如果带参数，那么该参数会被忽略)。它开始执行并返回第一个`yield "2+2=?"`的结果。此时，generator 执行暂停，而停留在`(*)`行上。
...
![picture](../assert/imgs/generator6.png)
