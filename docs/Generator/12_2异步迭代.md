...
这种模式非常常见。不仅可用于用户列表，这种模式还可以用于任意东西。
...
让我们创建一个函数`fetchCommits(repo)`，用来在任何我们需要的时候发出请求，来为我们获取 commit。并且，该函数能够关注到所有分页内容。对于我们来说，它将是一个简单的`for await..of`异步迭代。
...
async function* fetchCommit(repo) {
  ...
  while (url) {
    ...
    const body = await reponse.json(); // (2) 响应的是 JSON (array of commits)

    // (3) 前往下一页的 URL 在header 中，提取它
    let nextPage = reponse.headers.get("Link").match(/<(.*?)>; rel="next"/);
    nextPage = nextPage?.[1];
...
1. 我们使用浏览器的**fetch**方法来下载 commit。
   ...
   - 初始 URL 是
     ...
     并且下一页的 URL 将在响应的`Link`header 中。
...
3. 我们应该从响应(response)的`Link`header 中获取前往下一页的 URL。它有一个特殊的格式，所以我们对它使用正则表达式。
   ...
   - 前往下一页的 URL 看起来可能像这样`https://api.github.com/repositories/93253246/commits?page=2`。这是由 Github 自己生成的。

4. 然后，我们将接收到的所有 commit 一个一个地 yield 出来，当所有的 commit 都 yield 完成时，将触发下一个`while(url)`迭代，并发出下一个请求。
...
对于我们来说，它只是一个返回 commit 的异步 generator。
...
| :----------------------: | :----------------------------: | :-----------------------------------------------: |
| 提供 iterator 的对象方法 |       `Symbol.iterator`        |              `Symbol.asyncIterator`               |
|    `next()`返回的值是    | `{value:..., done:true/false}` | resolve 成`{value:...,done:true/fasle}`的 Promise |

异步 generator 与常规的 generator 在语法上的区别:
...
我们可以使用异步 generator 来处理此类数据。值得注意的是，在一些环境中，例如浏览器环境下，还有另一个被称为 Streams 的 API，它提供了特殊的接口来处理此类数据流(例如，从一个地方下载并立即发送到其他地方)。
